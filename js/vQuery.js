...
Base.prototype.getClass=function(classname,parent){
	var eles=null,
		...
		node=null;
	 	...
	 	for(var i=0;i<eles.length;i++){
	 		if((new RegExp('(\\s|^)' +classname +'(\\s|$)')).test(eles[i].className)){
...
Base.prototype.addClass=function(classname){
	for(var i=0;i<this.elements.length;i++){
		if(this.elements[i].className==""){
			...
			if((new RegExp('(\\s|^)' +classname +'(\\s|$)')).test(this.elements[i].className)==false){
				this.elements[i].className+=" "+classname;
			}
		}
	}
...
Base.prototype.opacity=function(num){
	...
	for(var i=0;i<this.elements.length;i++){
			if(arguments.length==1){
				...
				return getStyle(this.elements[i],"opacity");
			}
...
Base.prototype.center=function(){
	...
	for(var i=0;i<this.elements.length;i++){
		var left=getScroll().left+(getInner().width-this.elements[i].offsetWidth)/2,
			top=getScroll().top+(getInner().height-this.elements[i].offsetHeight)/2;
...
Base.prototype.animat=function(obj){
	...
	for(var i=0;i<this.elements.length;i++){
		...
		if(attr=="opacity"){
			...
			speed=target>ele[attr]? speed:-speed;
		}else{
		...
		ele.move=setInterval(function(){
			...
			for(var a in mul){
				...
				if(type=="buffer"){
					if(attr=="opacity"){
						speed=(target-parseInt(getStyle(ele,attr)*100))/v;
					}else if(attr=="scrollTop"){
						speed=(target-ele[attr])/v;
					}else{
						speed=(target-getStyle(ele,attr))/v;
					}
				...
				if(attr=="opacity"){
					...
					iNowStyle=parseInt(ele[attr])+speed;
				}else{
